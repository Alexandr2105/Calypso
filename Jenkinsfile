...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3031
        NAMESPACE = "kustogram-site"
        REGISTRY_HOSTNAME = "alex2105"
        PROJECT = "calypso-full"
        REGISTRY = "registry.hub.docker.com"
        DEPLOYMENT_NAME = "calypso-full-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                        docker.withRegistry("https://${env.REGISTRY}", 'kustogram-site') {
